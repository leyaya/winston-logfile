...
async function index(ctx) {
	...
	ctx.body = ejs.render(html, { arylist: arylist });
}
...
async function detail(ctx) {
	...
	namelist.forEach(filename => {
		promiselist.push(readFiletoStr(path.resolve('../log/' + filename)));
	})
		...
		if (item) {
			...
			if (dishItem) {
				...
				if (cityItem) {
					...
					let city = {};
					dishItem[obj.cityname] = { count: 1 };
				}
			} else {
				let cityobj = {};
				cityobj[obj.cityname] = { count: 1 };
				gather[obj.keyword] = cityobj;
			}
		}
	});
	const html = detailtemp();
	ctx.body = ejs.render(html, { gather: gather });
}
